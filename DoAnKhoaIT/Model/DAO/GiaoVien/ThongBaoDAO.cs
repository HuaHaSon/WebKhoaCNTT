    ...
    public class ThongBaoDAO
    {
        ...
        public IEnumerable<Baidang> ListALL(int page, int pagesize, string txtsearch, DateTime? tungay, DateTime? denngay, string tieude, string noidung, string loaibd)
        {
            ...
            if (tungay == null && denngay == null && tieude == null && noidung == null && loaibd == null)
            {
                if (txtsearch == null)
                {
                    return db.Baidangs.Where(s => s.Flag == true && (s.ĐoituongHT == "GV" || s.ĐoituongHT == "ALL") && s.Maloai == "TB").OrderByDescending(a => a.Ngaydang).ToPagedList(page, pagesize);
                }
                else
                {
            ...
            {
                var query = db.Baidangs.Where(s => s.Flag == true && (s.ĐoituongHT == "GV" || s.ĐoituongHT == "ALL"));
